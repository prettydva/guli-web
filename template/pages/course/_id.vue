  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      <section class="path-wrap txtOf hLh30">
        ...
        \
        <a href="#" title class="c-999 fsize14">{{course.subjectCategory}}</a>
        \
        <span class="c-333 fsize14">{{course.subject}}</span>
      </section>
      <div>
        ...
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt" v-else>
              <a href="#" @click="createOrder(course.id)" title="立即购买" class="comm-btn c-btn-3">立即购买</a>
            </section>
          </section>
        </aside>
  ...
  methods: {
    ...
    initCourseInfo() {
      courseApi.getCourseInfo(this.courseId).then((resp) => {
        console.log("data:" + JSON.stringify(resp.data.data));
    ...
    createOrder() {
      orderApi.createOrder(this.courseId).then((resp) => {
        //获取返回订单号
        //跳转到订单页面
        this.$router.push({ path: `/order/${resp.data.data}` });
      });
    },
  },
};
