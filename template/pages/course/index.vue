...
export default {
  ...
  methods: {
    ...
    //4 点击某个一级分类，查询对应二级分类
    searchOne(subjectCategoryId, index) {
      ...
      this.searchObj.subjectCategoryId = subjectCategoryId;
      //点击某个一级分类进行条件查询
      this.gotoPage(1);
      ...
      for (let i = 0; i < this.subjectNestedList.length; i++) {
        ...
        //比较id是否相同
        if (subjectCategoryId == oneSubject.id) {
          //从一级分类里面获取对应的二级分类
          this.subSubjectList = oneSubject.subjects;
        }
      }
    ...
    searchBuyCount() {
      //设置对应变量值，为了样式生效
      this.setSearchObj("buyCountSort");
      //调用方法查询
      this.gotoPage(1);
    },
    //7 最新排序
    searchGmtCreate() {
      //设置对应变量值，为了样式生效
      this.setSearchObj("gmtCreateSort");
      //调用方法查询
      this.gotoPage(1);
    },
